...
function cleanUp() {
  ...
  if [[ "$fs_setting" == "true" ]] && pulseAudio hide
    ...
    if [[ "$MODE" == "dump" ]]; then
      ...
      if [[ "$DESTINATION" == "es" ]]; then
        ...
        if [[ "$REQUESTED_SESSION" == 0 ]]; then
          ...
          if [[ "$SPEED" == "fast" ]] && sed -i '/audio_driver =/c\audio_driver = sdl2' /opt/retropie/configs/all/retroarch.cfg
